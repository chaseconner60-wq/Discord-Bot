...
async function createTicket(guildId, { channelId, openerId, openerTag }) {
  const { rows } = await pool.query(
    `INSERT INTO tickets (guild_id, ticket_number, channel_id, opener_id, opener_tag, status, created_at)
     VALUES ($1, (SELECT COALESCE(MAX(ticket_number), 0) + 1 FROM tickets WHERE guild_id = $1), $2, $3, $4, 'open', $5)
     RETURNING ticket_number AS "ticketNumber", channel_id AS "channelId", opener_id AS "openerId",
               opener_tag AS "openerTag", status, created_at AS "createdAt"`,
    [guildId, channelId, openerId, openerTag, Date.now()]
  );
  return rows[0];
}
...
async function getTicketByChannel(guildId, channelId) {
  const { rows } = await pool.query(
    `SELECT ticket_number AS "ticketNumber", channel_id AS "channelId", opener_id AS "openerId",
            opener_tag AS "openerTag", status, claimed_by_tag AS "claimedByTag",
